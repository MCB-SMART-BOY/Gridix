//! SQLite 类型化运行时 E2E 集成测试
//!
//! 使用 REAL 代码路径（execute_typed、apply_mutations、load_catalog）
//! 对真实 SQLite 数据库文件执行端到端验证。无 mocking。
//!
//! 覆盖：
//! - 复合主键 CRUD 往返
//! - NULL / 空字符串区分
...
async fn null_and_empty_string() {
    ...
    let outcome = execute_typed(
        &config,
        "CREATE TABLE nullable_test (id INT PRIMARY KEY, val TEXT)",
    )
    .await
    .unwrap();
    assert_affected(outcome, 0);
...
async fn default_value() {
    ...
    // 2. INSERT without specifying created_at — use InputValue::Unspecified
    let insert_batch = MutationBatch {
        mutations: vec![Mutation::Insert {
...
async fn catalog_load() {
    ...
    execute_typed(
        &config,
        "CREATE TABLE products (id INTEGER PRIMARY KEY, name TEXT, price REAL DEFAULT 0.0)",
    )
    .await
    .unwrap();
    ...
    assert!(orders.schema.is_none(), "SQLite has no schema qualifier");
    ...
    assert!(pk.name.is_none(), "SQLite PK names are unnamed");
