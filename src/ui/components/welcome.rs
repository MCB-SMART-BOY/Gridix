...
impl Welcome {
        ...
        ui.horizontal(|ui| {
            ...
            ui.allocate_ui_with_layout(
                ...
                |ui| {
                    ...
                    if action.is_none() {
                        action = Self::show_database_cards(ui, content_width, status);
                    } else {
                        Self::show_database_cards(ui, content_width, status);
                    }
                    ui.add_space(SPACING_LG);

                    if action.is_none() {
                        action = Self::show_quick_start(ui);
                    } else {
                    ...
                    if action.is_none() {
                        action = Self::show_onboarding_flow(ui, onboarding);
                    } else {
                        Self::show_onboarding_flow(ui, onboarding);
                    }
                ...
                ui.with_layout(egui::Layout::top_down(egui::Align::Center), |ui| {
                    ...
                    match status {
                        ...
                        WelcomeServiceState::BuiltIn | WelcomeServiceState::Running => {
                            ui.label(RichText::new("点击卡片创建连接").size(11.5).color(MUTED));
                        }
                    }
                });
            })
    ...
    /// 显示快速开始提示
    fn show_quick_start(ui: &mut egui::Ui) -> Option<WelcomeAction> {
        let mut action = None;
        egui::Frame::NONE
            ...
            .show(ui, |ui| {
                ui.with_layout(egui::Layout::top_down(egui::Align::Center), |ui| {
                    ...
                    ui.add_space(8.0);
                    ui.label(
                        RichText::new("如果是想要入门操作和学习数据库相关概念，请点击左上角小问号")
                            .color(GRAY),
                    );
