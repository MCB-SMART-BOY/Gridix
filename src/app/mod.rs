...
impl DbManagerApp {
    ...
    /// 生成新的连接请求 ID
    fn next_connect_request_id(&mut self) -> u64 {
        self.next_connect_request_id = self.next_connect_request_id.wrapping_add(1);
        if self.next_connect_request_id == 0 {
            self.next_connect_request_id = 1;
        }
        self.next_connect_request_id
    }

    /// 生成新的元数据请求 ID
    fn next_metadata_request_id(&mut self) -> u64 {
        self.next_metadata_request_id = self.next_metadata_request_id.wrapping_add(1);
        if self.next_metadata_request_id == 0 {
            self.next_metadata_request_id = 1;
        }
        self.next_metadata_request_id
    }

    /// 根据当前活动连接的 pending 请求刷新 connecting 标记
    fn refresh_connecting_flag(&mut self) {
    ...
    /// 查询完成后清理任务跟踪
    fn finalize_query_task(&mut self, request_id: u64) {
    ...
    /// 取消指定查询请求
    fn cancel_query_request(&mut self, request_id: u64) {
    ...
    /// 取消某个连接关联的所有查询请求
    fn cancel_queries_for_connection(&mut self, conn_name: &str) {
