...
impl DbManagerApp {
    ...
    pub(super) fn set_theme(&mut self, ctx: &egui::Context, preset: ThemePreset) {
    ...
    /// 保存当前连接的历史记录到配置
    pub(super) fn save_current_history(&mut self) {
    ...
    /// 加载指定连接的历史记录
    pub(super) fn load_history_for_connection(&mut self, conn_name: &str) {
        // 先保存当前连接的历史
        self.save_current_history();
