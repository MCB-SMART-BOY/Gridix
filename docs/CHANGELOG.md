...
## [Unreleased]
- Peer-review fixes for the runtime guards and the theme/responsive work:
  - A late connect reply from a superseded connection instance can no longer overwrite the replacement. Editing a connection's config rebuilds it under the same name with a new `ConnectionId` and a new `OperationKey::Connect(id)`, so the old task still counts as current for its own key; `handle_connected_with_tables` and `handle_database_selected` now check the connection id carried by the outcome (`does_runtime_connection_match`), matching the metadata and active-tables handlers. Proof: `connected_event_from_superseded_connection_instance_is_ignored`.
  - Disconnecting a non-active connection now releases the metadata loading flags together with the pending request, so the trigger/routine spinner cannot stay on after the reply is dropped (`disconnect_releases_metadata_loading_for_inactive_connection`).
  - The per-handler `pending_*` presence guards were unreachable (the event arm already gates the call) and were removed; the event arm is the single authoritative gate.
  - Follow-system theme: a restart no longer leaves the startup preset in place when it contradicts `is_dark_mode` (`follow_mode_reapplies_stored_mode_after_restart`), an explicit dark/light toggle turns following off instead of flickering for one frame, and `ThemeManager::apply` pins the egui theme slot so a system flip cannot switch egui to a style Gridix never wrote.
  - Responsive row thresholds are now single-sourced in `src/ui/dialogs/responsive.rs` (`DdlDialog`/`ImportDialog` reuse them), wide/medium labels truncate to their column, and the headless measurements assert the rendered control width instead of only the row total.

  `[Unreleased]` 的复核修复：陈旧的连接回包不再能覆盖同名重建后的新连接（按回包携带的连接 id 判定）；断开非活跃连接会同时释放元数据加载标记，避免转圈卡死；移除了处理器内不可达的 pending 存在性守卫（事件分支是唯一权威门）；跟随系统主题在重启后会被纠正、显式亮暗切换会关闭跟随、`ThemeManager::apply` 钉住 egui 主题槽；响应式行阈值改为单源（`DdlDialog`/`ImportDialog` 复用）、宽档标签按列宽截断，无头测量同时断言控件实际宽度。
- The `Quality Gate` secret scan was failing on build output, not on committed secrets: the pinned gitleaks v8.24.2 silently ignored the `[[allowlists]]` table and reported PEM-format strings inside `russh`/`pkcs8` `.rmeta` files (proved with a fixture: the same file was reported under `target/` and under `src/`). v8.30.1 honors that table (gitleaks added it in 8.25.0), so the repository's `paths` allowlist now actually skips `target/` and `.direnv/`; the workflow command is unchanged and a local run with the pinned digest reports no leaks over the working tree.
  `Quality Gate` 的密钥扫描失败源于构建产物而非已提交的密钥：固定版 gitleaks v8.24.2 会静默忽略 `[[allowlists]]` 表，把 `russh`/`pkcs8` 的 `.rmeta` 文件中的 PEM 文本报为泄漏（用夹具证明：同一文件在 `target/` 与 `src/` 下都被报出）。v8.30.1 支持该表（gitleaks 自 8.25.0 起提供），因此仓库 `.gitleaks.toml` 的 `paths` allowlist 现在真正生效并跳过 `target/` 与 `.direnv/`；工作流命令未变，用该 digest 在本地扫描当前工作树已无发现。
- Dialog form rows no longer overflow narrow viewports. `CreateDbDialog`, `CreateUserDialog`, and `ExportDialog` were the last surfaces still laying out label/control rows with `ui.horizontal` plus fixed `desired_width`; they now render through the shared helpers in `src/ui/dialogs/responsive.rs` (row-width classes, per-dialog `RowMetrics`, thin wrappers so call sites stay unchanged), and the remaining option rows use `ui.horizontal_wrapped`. Proof is by headless render measurement — `export_option_rows_fit_narrow_viewport` fails on the old layout at 200 px and passes after the change.
  对话框表单行不再在窄视口下横向溢出。`CreateDbDialog`、`CreateUserDialog` 与 `ExportDialog` 是最后仍用 `ui.horizontal` + 固定 `desired_width` 排版「标签 + 控件」行的界面；它们改为走 `src/ui/dialogs/responsive.rs` 的共享助手（档位阈值、每个对话框的 `RowMetrics`、薄包装保持调用点不变），其余选项行改用 `ui.horizontal_wrapped`。证据是无头渲染测量：`export_option_rows_fit_narrow_viewport` 在旧实现下于 200 px 失败，改动后通过。
- Query cancellation is now connection-scoped. `OperationKey::Query` carries `{ connection, document }`, so disconnecting one connection no longer cancels another connection's in-flight queries; `cancel_queries_for_connection` takes a `ConnectionId` and `cancel_queries_for_document` covers the tab-scoped path. Covered by `cancel_queries_for_connection_only_cancels_that_connection` and `cancel_queries_for_document_ignores_connection`.
  查询取消改为按连接隔离。`OperationKey::Query` 现在携带 `{ connection, document }`，断开某个连接不会再取消其它连接的在途查询；`cancel_queries_for_connection` 接收 `ConnectionId`，`cancel_queries_for_document` 覆盖按 Tab 取消的路径。由 `cancel_queries_for_connection_only_cancels_that_connection` 与 `cancel_queries_for_document_ignores_connection` 覆盖。
- Removed the always-true request-id guards. The four guards in `src/app/runtime/handler.rs` compared an id against the same value they were dispatched with, and the inert id fields plus `next_connect_request_id`/`next_metadata_request_id` are gone: `pending_*` now only answers "is a request in flight", while `TaskRegistry::is_current()` and `metadata_context_matches_current` decide freshness. <!-- doc-symbols: ignore: names fields that were deleted -->
  删除恒为真的请求号守卫。`src/app/runtime/handler.rs` 中四处守卫把请求号与派发时的同一值比较，惰性 id 字段以及 `next_connect_request_id`/`next_metadata_request_id` 一并移除：`pending_*` 只回答「是否有在途请求」，新鲜度由 `TaskRegistry::is_current()` 与 `metadata_context_matches_current` 判定。
- Trigger and routine metadata tasks now attach their `JoinHandle` and race the request against their cancellation token, so superseding a metadata load or switching connections ends it cooperatively instead of waiting for the internal timeout.
  触发器与存储过程元数据任务现在会 attach `JoinHandle`，并让请求与取消令牌竞争，因此被取代或连接切换时协作结束，而不是等内部超时。
- Added the system theme follow mode (`AppConfig.theme_follows_system`). `core::theme::resolve_dark_mode` is the pure decision function (`None` from the platform keeps the current mode), `sync_system_theme` runs once per frame and rebuilds styles whenever the applied theme differs from the resolved mode (including the preset a restart left behind), and the theme chooser exposes a "跟随系统主题" toggle; confirming a theme there and the Ctrl+D toggle both turn the follow mode off so the explicit choice is not overridden on the next frame.
  新增系统主题跟随模式（`AppConfig.theme_follows_system`）。`core::theme::resolve_dark_mode` 是纯决策函数（平台未提供系统主题时保持当前模式），`sync_system_theme` 每帧运行，只要生效主题与解析结果不一致就重建样式（包括重启后由 `theme_preset` 留下的残留）；主题选择器提供「跟随系统主题」开关，在该对话框确认主题与 Ctrl+D 切换都会关闭跟随模式，避免显式选择在下一帧被系统覆盖。
- Removed dead snapshot loaders: `load_schema_snapshot` (`src/data/query/mod.rs`) and its SQLite-only helper `load_snapshot` had no in-tree consumer after the Schema diff cutover, where the dialog reads `SchemaSnapshot::from_catalog` directly. <!-- doc-symbols: ignore: both symbols were deleted -->
  删除死代码快照加载器：Schema diff 切换后，`load_schema_snapshot`（`src/data/query/mod.rs`）与其 SQLite 专用助手 `load_snapshot` 已无仓库内消费者——对话框直接读取 `SchemaSnapshot::from_catalog`。
- Fixed the two gates that were keeping `main` red. `Backend TLS Acceptance` never started its PostgreSQL server: the job chowned the TLS key to `999:999`, but the pinned digest is an Alpine image whose `postgres` user is uid 70, so PostgreSQL rejected the key ("must be owned by the database user or root"); the fixture now derives the uid from the image and pins that digest in one place. The `Dependency policy audit` step aborted with "unsupported CVSS version: 4.0" because its pinned cargo-deny (2025-06-11) and cargo-audit (2025-02-28, `cvss 2.0.0`) predate CVSS 4.0, so both pins move to the versions that parse the current advisory database (cargo-deny v0.20.2, cargo-audit v0.22.2).
  修复两处让 `main` 持续变红的门禁。`Backend TLS Acceptance` 的 PostgreSQL 服务从未启动：该作业把 TLS 私钥 chown 到 `999:999`，而固定 digest 是 Alpine 镜像、其 `postgres` 用户为 uid 70，PostgreSQL 因此拒绝该私钥（"must be owned by the database user or root"）；fixture 改为从镜像派生 uid，并只在一处固定该 digest。`Dependency policy audit` 以 "unsupported CVSS version: 4.0" 中断，原因是其固定的 cargo-deny（2025-06-11）与 cargo-audit（2025-02-28，`cvss 2.0.0`）早于 CVSS 4.0；两个 pin 均升级到能解析当前 advisory database 的版本（cargo-deny v0.20.2、cargo-audit v0.22.2）。
