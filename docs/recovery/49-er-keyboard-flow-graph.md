...
3. 当前局部快捷键集合是：
   ...
   - `l / Enter / Right`: open selected table into the main workspace
   - `h / Left / Esc`: return to the most recent legal non-ER workspace area
   - `q`: close ER
   - `r`: refresh
   - `Shift+L`: layout
   - `f`: fit view
...
8. 打开 ER 仍不会自动抢焦点；关闭 ER 时若当前焦点在 ER，会回退到最近一个合法的非 ER workspace 区域，并在目标不可用时回退到 `DataGrid`。
   - [src/app/input/input_router.rs](../../src/app/input/input_router.rs)
...
### 3.3 Open should not steal focus
...
- 打开 ER 是 reveal companion pane，不是 modal enter
- 用户仍应显式决定何时进入 ER scope
...
- bare `l` 不再用于 `layout`
- `l / Enter / Right` 现在统一表示“打开当前选中表”
- `Shift+L` 接管 relayout

当前结论：

- ER 键盘流已经切到更强的 TUI `h/l` 主线
- `h/j/k/l` 的默认语义现在只属于浏览态；视口平移已被明确压到 `er_diagram.viewport`
