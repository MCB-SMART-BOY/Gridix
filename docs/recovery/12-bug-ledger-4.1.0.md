...
- 症状:
  ...
  - ER 的生成链若继续只靠单一布局函数，后续很容易再次把“语义图、布局策略、视图密度、边显示”这几层职责重新混在一起。
- 最小复现方式:
  1. 在浅色主题下打开 ER 图。
...
- 相关文件:
  ...
  - `src/ui/components/er_diagram/graph.rs`
  - `src/ui/mod.rs`
  - `docs/recovery/10-master-recovery-plan.md`
  - `docs/recovery/11-core-flows-and-invariants.md`
...
- 证据:
  ...
  - `src/ui/components/er_diagram/render.rs` 现已把表卡重画成更贴近 Gridix 工作台语言的轻量结构：左对齐标题、右侧 compact stats、`Standard / KeysOnly` 两档列密度，以及焦点邻域 dimming，不再只靠旧的统一厚卡片样式承载所有信息。
  ...
  - 基于 live 反馈，本轮继续收口默认导航合同：bare `h / l` 已不再绑定到 `er_diagram.back` / `er_diagram.open_selected`，而是回收到 `er_diagram.geometry_left / geometry_right`；`Enter / Right` 继续作为显式打开当前表的动作，`Left / Esc` 继续作为显式返回主工作区的动作，避免聚焦 ER 时刚开始按 `h/l` 就立刻离开 ER 或触发查询成功提示。
...
- 当前执行顺序:
  ...
  10. `l` contract 第十刀已完成后又被 live 反馈修正：bare `l` 现已从“打开当前表”回收到 ER 内部右向几何邻接，`Enter / Right` 继续承担显式打开当前表，`Shift+L` 继续承担 relayout
  ...
  17. viewport-mode 第十七刀已落地：ER 键盘流现已拆成 `er_diagram` 浏览子作用域与 `er_diagram.viewport` 视口子作用域；`v` 在两者间切换，浏览态继续承载 `j/k` 线性浏览、`h/l` 左右几何邻接、`Enter/Right` 打开当前表与 `q` 关闭，视口态将 `h/j/k/l` 收回为平移，`Esc` 只退出视口模式回到浏览态
  ...
  28. relation-first default layout 第二十八刀已落地：ER 打开后的加载骨架仍先走稳定 grid，但 finalize 现在会在显式或推断关系 ready 后自动走关系层级种子 + force-directed refine；只有空关系图才保持 grid，因此“关系布局”不再只靠手动 `Shift+L` 才能看见。手动 relayout 现在也复用同一条关系优先路径。
  29. relation-seeded sibling ordering 第二十九刀已落地：`hierarchical_layout()` 的同层兄弟表现在会先按名称做稳定初始化，再按已知关系邻居的重心做上下 sweep，因此关系优先布局不再继续把原始输入顺序直接投射成同层横向顺序；相关 focused tests 已锁住“按父表重心排序”和“同重心时名称回退”的 contract。
  30. relation-first live observation 第三轮已完成：在学习样例默认完成态下，当前关系种子 + 同层重心排序未复现明显“同层兄弟表直接继承输入顺序”的反例，主簇分组观感已比纯 grid skeleton 更贴近关系结构；当前继续保持“观察 / 启发式调优候选”，不继续猜测性下调 `force_directed_layout()` 参数。
  31. viewport-mode toggle de-dup 第三十一刀已落地：`v` 的视口模式切换现在只由 input router 路由到 `ToggleViewportMode`，ER render 不再重复消费同一个局部快捷键；整帧 focused test 已锁住“每次按 `v` 只切一次”。
  32. overlap-aware relation layout 第三十二刀已落地：`hierarchical_layout()` 现在按真实表卡尺寸计算层间和同层间距，`force_directed_layout()` 也改为基于表卡中心和实际尺寸做分离；相关 focused tests 已锁住“大表层级种子不重叠”和“大表关系完成态不重叠”的最小边界。
  33. component-aware relation seeding 第三十三刀已落地：`relationship_seeded_layout()` 现在会先把断开的关系簇和孤立表拆成独立组件，再分别做层级种子并按组件边界留白后进入全局 `force_directed_layout()`；相关 focused tests 已锁住“断开簇彼此分离”和“孤立表不再挤进相关主簇”的最小边界。
  34. command-palette toggle anchor 第三十四刀已落地：app-level focused test 现已锁住 `命令面板 -> toggle_er_diagram -> Enter` 会关闭 palette、分发 `ToggleErDiagram`、打开 ER 并切入 `FocusArea::ErDiagram`；因此当前 live 中命令面板自动注入失败继续视为环境噪声，而不是已证实的 ER owner 回归。
  ...
  42. ER generation redesign 第二阶段第四刀已落地：当 partial-reload 里的新增表和这些已恢复旧表本身存在关系时，当前不再只是“避让旧表”，而会先按关系邻居为它们选择局部锚点，再做最后一层几何避让；这样新增表不再轻易被甩在旧簇很远的位置，partial-reload 的观感更接近增量插入而不是整图重算。
  43. ER generation redesign 第二阶段第五刀已落地：当 partial-reload 里的新增表与已恢复旧表存在明确父/子方向关系时，当前不再只是“无方向地贴近邻居”，而会优先遵守层级语义：引用父表的新表优先落在父表下方，被子表引用的新表优先落在这些子表上方；这样增量插入不再轻易把子表甩到父表左右两侧。
  44. ER generation redesign 第二阶段第六刀已落地：当 partial-reload 里的新增表同时连到已恢复父表和子表时，当前不再在双向关系里退化成“直接掉到父层下面”；局部插入现在会优先保留桥接表所在的上下层带，并在四向候选里先看总重叠、再尽量保持原锚点垂直带，因此桥接表会更稳定地落在父/子层之间，而不是继续被挤到整个子层下面。
