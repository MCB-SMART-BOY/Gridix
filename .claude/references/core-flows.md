...
| 4 | **Stale response guard** | `RuntimeEvent` carries `TaskId` + `OperationKey`; `TaskRegistry::is_current()` discards stale completions. Legacy request-ID/pending-query handling is historical migration context. |
